...
import { Bot, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, LogOut, History, Cpu, Battery, Activity, Target, MapPin } from 'lucide-react';
...
const Dashboard = ({ user, setUser }) => {
  ...
  // Helper check to prevent repetitive logic
  ...
  const handleMove = async (direction) => {
    // ✅ Safeguard: Stop execution if viewer tries to bypass UI locks
      ...
      <div className="flex-none flex justify-between items-center mb-6 bg-slate-900 p-5 rounded-[2rem] shadow-2xl border border-slate-800">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-3">
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded-full border ${robotOnline ? 'border-green-500/50 text-green-400' : 'border-red-500/50 text-red-400'}`}>
              {backendStatus}
            </span>
          </div>
          <div className="flex gap-6 border-l border-slate-800 pl-8 font-mono text-xs uppercase">
             ID: <span className="text-blue-400">{user.username}</span> | Access: <span className={isViewer ? "text-slate-400" : "text-amber-500"}>{user.role}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-4 bg-slate-950 px-4 py-2 rounded-2xl border border-slate-800">
             ...
             <span className="text-xs font-black uppercase text-blue-400">{telemetry.state}</span>
          </div>
            ...
            <form onSubmit={handleGoTo} className="space-y-3">
              <div className="flex gap-3">
                <input 
                  ...
                  className="flex-1 bg-slate-950 border border-slate-800 rounded-xl p-2 text-center text-xs font-bold outline-none focus:border-blue-500 disabled:opacity-30" 
                  min="0" max="20" placeholder="X" 
                  disabled={isViewer}
                />
                <input 
                  type="number" 
              ...
              <button 
                ...
                className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black text-[10px] py-3 rounded-xl transition-all disabled:opacity-20 flex items-center justify-center gap-2 shadow-xl uppercase tracking-widest"
              >
...
const NavButton = ({ icon, onClick, disabled }) => (
  <button 
    onClick={onClick} 
    disabled={disabled} 
    className="aspect-square flex items-center justify-center bg-slate-800 rounded-xl text-slate-300 hover:bg-blue-600 hover:text-white transition-all disabled:opacity-10 border border-slate-700 p-2 active:scale-90"
  >
